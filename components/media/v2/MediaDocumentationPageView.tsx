...
interface MediaDocumentationPageViewProps {
  title: string;
  icon: LucideIcon;
  content: DocumentationPageContent;
}

export function MediaDocumentationPageView({
  title,
  icon,
  content,
}: MediaDocumentationPageViewProps) {
  ...
  return (
    <AudioPageShell
      title={title}
      description={content.subtitle ?? "Media Department reference"}
      icon={icon}
      breadcrumbs={[
        { label: "Media Department", href: "/media" },
        { label: "Documentation", href: "/media/documentation" },
        { label: title },
      ]}
      compactMobile
      backHref="/media"
