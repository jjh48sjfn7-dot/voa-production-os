...
interface LightingEquipmentDetailContentProps {
  slug: string;
}

export function LightingEquipmentDetailContent({
  slug,
}: LightingEquipmentDetailContentProps) {
  ...
  const category = getLightingEquipmentCategory(equipment.categoryId);
  const backHref = category?.href ?? "/lighting/equipment";
